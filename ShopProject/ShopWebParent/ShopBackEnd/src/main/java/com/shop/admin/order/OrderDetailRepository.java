package com.shop.admin.order;

import java.util.Date;
import java.util.List;

import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.CrudRepository;

import com.shop.common.entity.Order;
import com.shop.common.entity.OrderDetail;

public interface OrderDetailRepository extends CrudRepository<Order, Integer> {

	@Query("SELECT NEW com.shop.common.entity.OrderDetail(d.product.category.name, d.quantity,"
			+ " d.productCost, d.shippingCost, d.subTotal)"
			+ " FROM OrderDetail d WHERE d.order.orderTime BETWEEN ?1 AND ?2")
	public List<OrderDetail> findWithCategoryAndTimeBetween(Date startTime, Date endTime);

	@Query("SELECT NEW com.shop.common.entity.OrderDetail(d.quantity, d.product.name,"
			+ " d.productCost, d.shippingCost, d.subTotal)"
			+ " FROM OrderDetail d WHERE d.order.orderTime BETWEEN ?1 AND ?2")
	public List<OrderDetail> findWithProductAndTimeBetween(Date startTime, Date endTime);	
}
